...
def api_show_auth_providers(client_id: str = ""):
    url = f"{BASE_URL}/oocmgr/oidc/show-auth-providers"
    payload = {
        "tenant_id": TENANT_ID,
        "client_id": client_id,
    }
...
def api_list_saml_providers():
    url = f"{BASE_URL}/oocmgr/saml/list-providers"
    payload = {
        "tenant_id": TENANT_ID,
    }
    log(f"[authsec] POST {url}")
...
def api_add_saml_provider(
    ...
    url = f"{BASE_URL}/oocmgr/saml/add-provider"
    payload = {
        "tenant_id": TENANT_ID,
        "client_id": client_id,
...
def api_configure_auth_provider(
    client_id: str,
    provider_name: str,
    display_name: str,
    callback_url: str,
    ...
    url = f"{BASE_URL}/oocmgr/oidc/edit-client-auth-provider"
    payload = {
        "tenant_id": TENANT_ID,
        "client_id": client_id,
        "provider_name": provider_name,
        ...
        "is_active": is_active,
        "provider_config": provider_config or {"auth_url": "", "token_url": ""},
...
async def list_tools():
    return [
        ...
        types.Tool(
            name="show_auth_providers",
            description="Show available authentication providers for OIDC / OAuth 2.0 auth setup. Use an empty client_id to view the tenant-wide aggregated provider view.",
            inputSchema={
                "type": "object",
                "properties": {
                    "client_id": {
                        "type": "string",
                        "description": "Optional client id for client-specific provider view. Leave empty for aggregated tenant view.",
                    }
                },
            },
        ),
        types.Tool(
            ...
            description="SSO Configuration: configure or enable an authentication provider for a client. First ask which SSO provider setting the user wants to configure, such as Google, GitHub, Microsoft, or another provider. Then collect the client and callback details.",
            inputSchema={
                "type": "object",
                "properties": {
                    "client_id": {
                        "type": "string",
                        "description": "Client id to configure the auth provider for",
                    },
                    "provider_name": {
                        "type": "string",
                        "description": "Ask this as the SSO provider setting to configure, for example google, github, microsoft, or authsec",
                    },
                    "display_name": {
                        "type": "string",
                        "description": "Provider display name, for example Google, GitHub, or Microsoft",
                    },
                    "callback_url": {
                        "type": "string",
                    ...
                    "is_active": {
                        "type": "boolean",
                        "description": "Whether the provider should be enabled for the client",
                    },
                    "provider_config": {
                        "type": "object",
                        "description": "Optional provider-specific config object. For the captured GitHub flow, auth_url and token_url were empty strings.",
                    },
                },
                "required": ["client_id", "provider_name", "display_name", "callback_url"],
            },
        ),
        types.Tool(
            name="list_saml_providers",
            description="SSO Configuration: list SAML 2.0 providers configured for the tenant.",
            inputSchema={
                "type": "object",
                "properties": {},
            },
        ...
        types.Tool(
            name="add_saml_provider",
            description="SSO Configuration: add or configure a SAML 2.0 provider for a client. First ask for the client, then collect the SAML provider settings.",
            inputSchema={
                "type": "object",
                "properties": {
                    "client_id": {
                        "type": "string",
                        "description": "Client id to attach the SAML provider to",
                    },
                    "provider_name": {
                        "type": "string",
                        "description": "Internal SAML provider name",
                    },
                    "display_name": {
                        "type": "string",
                    ...
                    "certificate": {
                        "type": "string",
                        "description": "SAML X.509 certificate string",
                    },
                    "name_id_format": {
                        "type": "string",
                        "description": "SAML NameID format, for example urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress",
                    },
                    "attribute_mapping": {
                        "type": "object",
                        "description": "Optional attribute mapping object with keys like email, first_name, and last_name",
                    },
                    "is_active": {
                        "type": "boolean",
                        "description": "Whether the SAML provider should be enabled",
                    },
                    "sort_order": {
                        "type": "integer",
                        "description": "Optional sort order for provider display",
                    },
                },
                "required": [
                    "client_id",
        ...
        types.Tool(
            name="create_client",
            description="Onboard a new client in the AuthSec dashboard. Always ask in this order when details are missing: 1. client_type (choose MCP Server, AI Agent, or Claw Bot), 2. auth_method (choose basic_auth, oidc_oauth_2_0, or saml_2_0), 3. name, 4. type-specific fields. For ai_agent, ask for platform and then selectors. For claw_bot, ask whether the user wants to provide an optional redirect_url before creating it. If auth_method is OIDC / OAuth 2.0 or SAML 2.0, note that advanced auth setup is a follow-up step after client creation.",
            inputSchema={
                "type": "object",
                "properties": {
                    "client_type": {
                    ...
                    "auth_method": {
                        "type": "string",
                        "description": "Ask this second, after client_type is known. Authentication method to prepare for: basic_auth, oidc_oauth_2_0, or saml_2_0",
                        "enum": ["basic_auth", "oidc_oauth_2_0", "saml_2_0"],
                    },
                    "name": {
                        "type": "string",
...
async def call_tool(name: str, arguments: dict):
    ...
    if name == "show_auth_providers":
        client_id = arguments.get("client_id", "").strip()
        try:
            status_code, result = api_show_auth_providers(client_id)
            return [types.TextContent(type="text", text=json.dumps({
                "ok": status_code == 200,
                "status_code": status_code,
    ...
    if name == "configure_auth_provider":
        client_id = arguments.get("client_id", "").strip()
        provider_name = arguments.get("provider_name", "").strip()
        display_name = arguments.get("display_name", "").strip()
        callback_url = arguments.get("callback_url", "").strip()
        is_active = arguments.get("is_active", True)
        provider_config = arguments.get("provider_config")
        if not client_id or not provider_name or not display_name or not callback_url:
            return [types.TextContent(type="text", text=json.dumps({
                "configured": False,
                "error": "client_id, provider_name, display_name, and callback_url are required."
            }))]
        try:
            status_code, result = api_configure_auth_provider(
                client_id,
                provider_name,
                display_name,
                callback_url,
    ...
    if name == "list_saml_providers":
        try:
            status_code, result = api_list_saml_providers()
            return [types.TextContent(type="text", text=json.dumps({
                "ok": status_code == 200,
                "status_code": status_code,
                "result": result,
    ...
    if name == "add_saml_provider":
        client_id = arguments.get("client_id", "").strip()
        provider_name = arguments.get("provider_name", "").strip()
        display_name = arguments.get("display_name", "").strip()
        entity_id = arguments.get("entity_id", "").strip()
        sso_url = arguments.get("sso_url", "").strip()
        certificate = arguments.get("certificate", "").strip()
        name_id_format = arguments.get("name_id_format", "").strip()
        attribute_mapping = arguments.get("attribute_mapping")
        is_active = arguments.get("is_active", True)
        sort_order = arguments.get("sort_order", 1)
        if not all([client_id, provider_name, display_name, entity_id, sso_url, certificate, name_id_format]):
            return [types.TextContent(type="text", text=json.dumps({
                "configured": False,
                "error": "client_id, provider_name, display_name, entity_id, sso_url, certificate, and name_id_format are required."
            }))]
        try:
            status_code, result = api_add_saml_provider(
                client_id,
                provider_name,
                display_name,
                entity_id,
    ...
    if name == "create_client":
        ...
        auth_method_aliases = {
            "oidc_oauth_2_0": "oidc_oauth_2_0",
            "oidc": "oidc_oauth_2_0",
            "saml_2_0": "saml_2_0",
            "saml": "saml_2_0",
            "basic_auth": "basic_auth",
        }
        auth_method = auth_method_aliases.get(auth_method, auth_method)
        ...
        if auth_method not in ("basic_auth", "oidc_oauth_2_0", "saml_2_0"):
            return [types.TextContent(type="text", text=json.dumps({
                "created": False,
                "error": "auth_method is required and must be one of: basic_auth, oidc_oauth_2_0, saml_2_0.",
            }))]
        if not client_type:
            return [types.TextContent(type="text", text=json.dumps({
                "created": False,
        ...
        try:
            ...
            if status_code in (200, 201):
                follow_up = None
                if auth_method in ("oidc_oauth_2_0", "saml_2_0"):
                    follow_up = (
                        f"Client created for auth_method '{auth_method}'. "
                        f"The next step is to configure {'OIDC / OAuth 2.0' if auth_method == 'oidc_oauth_2_0' else 'SAML 2.0'} settings for this client."
                    )
                return [types.TextContent(type="text", text=json.dumps({
                    "created": True,
